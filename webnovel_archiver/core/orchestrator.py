...
def archive_story( # Removed DEFAULT_WORKSPACE_ROOT default for workspace_root
    ...
    try:
        ...
        if source_prefix and source_specific_id:
            s_id = f"{source_prefix}-{source_specific_id}"
            logger.info(f"Generated source-specific story ID: {s_id}")
        else:
            logger.warning(f"Could not generate a fully prefixed source-specific ID. Fetcher: {type(fetcher).__name__}, Source ID: {source_specific_id}. Falling back to generic ID.")
            # Fallback to generic ID generation if prefix or specific ID is missing
            s_id = generate_story_id(url=story_url, title=metadata.original_title)
            logger.info(f"Generated generic story ID (fallback): {s_id}")

    except ValueError as e: # Raised by fetcher if ID parsing fails
        logger.warning(f"Failed to get source-specific ID from fetcher: {e}. Falling back to generic ID generation.")
        s_id = generate_story_id(url=story_url, title=metadata.original_title)
        logger.info(f"Generated generic story ID (fallback after error): {s_id}")
    except NotImplementedError: # If a fetcher hasn't implemented the method
        logger.warning(f"Fetcher {type(fetcher).__name__} has not implemented get_source_specific_id. Falling back to generic ID generation.")
        s_id = generate_story_id(url=story_url, title=metadata.original_title)
        logger.info(f"Generated generic story ID (fallback, not implemented): {s_id}")
    except Exception as e: # Catch any other unexpected error from get_source_specific_id
        logger.error(f"Unexpected error getting source-specific ID: {e}. Falling back to generic ID generation.", exc_info=True)
        s_id = generate_story_id(url=story_url, title=metadata.original_title)
        logger.info(f"Generated generic story ID (fallback, unexpected error): {s_id}")

    if not s_id: # Should not happen if generate_story_id has its own timestamp fallback
        logger.error("CRITICAL: Story ID could not be generated by any method. Aborting.")
        _call_progress_callback({"status": "error", "message": "Critical error: Failed to generate a story ID."})
        return None

    metadata.story_id = s_id # Set the final story_id in metadata
    logger.info(f"Final story ID to be used: {s_id}")
    ...
    progress_data["story_id"] = s_id # Ensure progress_data also uses the final s_id
